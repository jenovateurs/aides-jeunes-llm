...
Cet outil vit dans son propre dépôt Git ([`jenovateurs/aides-jeunes-llm`](https://github.com/jenovateurs/aides-jeunes-llm)) et doit être **cloné dans le dossier `tools/` du repo [`aides-jeunes`](https://github.com/betagouv/aides-jeunes)**.

---

## Étape 0 — Cloner les projets

```bash
# Repo principal (si pas déjà fait)
git clone git@github.com:betagouv/aides-jeunes.git
# Cet outil, dans tools/aj-llm
cd aides-jeunes/tools
git clone git@github.com:jenovateurs/aides-jeunes-llm.git aj-llm
cd aj-llm
```

> ⚠️ Le dossier **doit** s'appeler `aj-llm` et se trouver dans `tools/` : les chemins vers `data/` en dépendent.

Pour la suite : ouvrir un terminal dans `aides-jeunes/tools/aj-llm`.
